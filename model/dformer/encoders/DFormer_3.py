...
class BlockFFT(nn.Module):
    def __init__(self, dim, h, w, time_embed_dim, groups=8, ):
        ...
        self.complex_weight1 = nn.Parameter(torch.randn(dim, h, w // 2 + 1, 2, dtype=torch.float32) * 0.02)
        self.complex_weight2 = nn.Parameter(torch.randn(dim, h, w // 2 + 1, 2, dtype=torch.float32) * 0.02)
        # self.cat_conv = ConvModule(in_channels=dim * 2, out_channels=dim, kernel_size=1,
        #                            norm_cfg=dict(type='BN', requires_grad=True))
        # self.complex_weight = nn.Parameter(torch.normal(mean=0, std=0.01, size=(dim, h, w // 2 + 1, 2), dtype=torch.float32))
        # self.norm = nn.GroupNorm(groups, dim)
        # self.act = nn.SiLU()

    def forward(self, x, y, scale_shift=None):
        B, C, H, W = x.shape
        # x = torch.fft.fft2(x, dim=(2, 3), norm='ortho')
        x = torch.fft.rfft2(x, dim=(2, 3), norm='ortho')
        y = torch.fft.rfft2(y, dim=(2, 3), norm='ortho')
        # x = x + self.block(x)
        x = x * torch.view_as_complex(self.complex_weight1)
        y = y * torch.view_as_complex(self.complex_weight2)
        # x = self.cat_conv(torch.cat([x, y], dim=1))
        x = x + y
...
class DFormer(BaseModule):
    def __init__(self, in_channels=4, depths=(2, 2, 8, 2), dims=(32, 64, 128, 256), out_indices=(0, 1, 2, 3),
        ...
        # )
        # self.complex_weight = nn.Parameter(torch.randn(dims[0] // 2, 88, 88, dtype=torch.float32) * 0.02)
        # self.complex_weight_c = nn.Parameter(torch.randn(dims[0] // 2, 88, 88, dtype=torch.float32) * 0.02)
        # self.time_embed_dim = 352
        # self.time_embed = nn.Sequential(
        #     nn.Linear(self.time_embed_dim, 4 * self.time_embed_dim),
        #     nn.SiLU(),
        #     nn.Linear(4 * self.time_embed_dim, self.time_embed_dim),
        # )
        self.mask_proj_e = nn.Conv2d(1, dims[0] // 2, kernel_size=3, stride=4, padding=1)
        self.mask_proj_e.weight.data.zero_()
        self.mask_proj_e.bias.data.zero_()
        embedding_dim = dims[0] // 2
        ...
        # )
        #
        # self.FFT_fusion = BlockFFT(embedding_dim, 88, 88, None, 11)

        # self.mask_proj_e2 = nn.Sequential(
        #     nn.Conv2d(dims[0] // 2, dims[0], kernel_size=3, stride=2, padding=1),
        #     nn.BatchNorm2d(dims[0]),
        #     nn.GELU(),
        # )
        # self.mask_proj_e3 = nn.Sequential(
        #     nn.Conv2d(dims[0], 144, kernel_size=3, stride=2, padding=1),
        #     nn.BatchNorm2d(144),
        #     nn.GELU(),
        # )
        # self.mask_proj_e4 = nn.Sequential(
        #     nn.Conv2d(144, 288, kernel_size=3, stride=2, padding=1),
        #     nn.BatchNorm2d(288),
        #     nn.GELU(),
        # )
    ...
    def forward(self, x, x_e, cond):
        ...
        for i in range(4):
            ...
            if i == 0:
                # 试一下在各层都加
                # 把t通过scale shift加进来
                # cond = self.time_embed(timestep_embedding(cond, self.time_embed_dim))
                # cond = cond * self.complex_weight_c
                c_e = self.mask_proj_e(cond)
                # x_e = self.decouple1(x_e)
                # c_e = self.decouple2(c_e)
                # x_e = self.FFT_fusion(x_e, c_e)
                # c_e = self.mask_proj_e(cond.unsqueeze(1).unsqueeze(-1))
                # x_e = x_e * self.complex_weight
                x_e = x_e + c_e
                # x = x + self.mask_proj(cond)
            # elif i == 1:
            #     c_e = self.mask_proj_e2(c_e)
            #     x_e = x_e + c_e
            # elif i == 2:
            #     c_e = self.mask_proj_e3(c_e)
            #     x_e = x_e + c_e
            # elif i == 3:
            #     c_e = self.mask_proj_e4(c_e)
            #     x_e = x_e + c_e
            # ([10, 48, 88, 88])
            x = x.permute(0, 2, 3, 1)
...
def DFormer_Base(pretrained=None, drop_path_rate=0.1, **kwargs):  # 82.1
