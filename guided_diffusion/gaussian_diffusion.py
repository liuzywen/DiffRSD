...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        self.mask_coefficient = to_torch(np.sqrt(1. - self.alphas_cumprod) * betas)
    ...
    def q_sample(self, x_start, t, noise=None, cond=None):
        ...
        if noise is None:
            ...
            cond *= self.extract(self.mask_coefficient.to(cond.device), t, x_start.shape)
            noise += cond
        assert noise.shape == x_start.shape

        # log_snr = self.log_snr(t / 1000)
        # log_snr_padded = self.right_pad_dims_to(x_start, log_snr)
        # alpha, sigma = np.sqrt(log_snr_padded.sigmoid()), np.sqrt((-log_snr_padded).sigmoid())
    ...
    def ddim_sample_loop(
        ...
        for sample in self.ddim_sample_loop_progressive(
            ...
            final = sample
            all_inter_samples.append(sample["sample"].cpu())
            all_samples.append(sample["pred_xstart"].cpu())
            all_model_output.append(sample["model_output"].cpu())
        # return final["sample"]
        final["all_inter_samples"] = all_inter_samples
        final["all_samples"] = all_samples
        final["all_model_outputs"] = all_model_output
        return final

    def ddim_sample_loop_progressive(self, model, shape, noise=None, clip_denoised=True, denoised_fn=None,
        ...
        for i in indices:
            ...
            with th.no_grad():
                if cond_fn is not None:
                    tt = t * 100
                    RGB = model_kwargs["image"][:, :-1, :, :]
                    T = model_kwargs["image"][:, -1:, :, :]
                    T = T.repeat(1, 3, 1, 1)
                    _, embeddings = self.embed_model(RGB, T)
                    '''embeddings = [1, 1, 1, 1]
                    embeddings_rgb = cond_fn_rgb(img, tt, rgb)
                    embeddings_t = cond_fn_t(img, tt,T)
                    for idx, eb in enumerate(embeddings_t):
                        # ([6, 64, 64, 64])
                        # ([6, 128, 32, 32])
                        # ([6, 320, 16, 16])
                        # ([6, 512, 8, 8])
                        embeddings[idx] = torch.cat([eb, embeddings_rgb[idx]], dim=1)'''
