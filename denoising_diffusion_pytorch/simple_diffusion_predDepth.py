...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        assert pred_objective in {'v', 'eps',
                                  'x0'}, 'whether to predict v-space (progressive distillation paper) or noise'
    ...
    def p_mean_variance(self, x, cond, time, time_next, return_history=False):
        ...
        # _, _, pred = self.model(x, batch_log_snr, cond)
        ...
        # x_start.clamp_(-1., 1.)
        # x_noised_t,_ = self.q_sample(x_start,log_snr)
        x_start.clamp_(-1., 1.)
        # x_t_next = x - x_noised_t + x_noised_t_next
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, time, time_next, return_history=False):
        batch, *_, device = *x.shape, x.device

        model_mean, model_variance, x_start = self.p_mean_variance(x=x, cond=cond, time=time, time_next=time_next,
                                                                   return_history=return_history)
        # x = self.p_mean_variance(x=x, cond=cond, time=time, time_next=time_next, return_history=return_history)

        if time_next == 0:
            return model_mean, x_start

        noise = torch.randn_like(x)
        x_t_next = model_mean + sqrt(model_variance) * noise
        return x_t_next, x_start
        # return x

    def save_img(self, img, name, type):
        img.clamp_(-1., 1.)
        img = unnormalize_to_zero_to_one(img)
        img = (img[0, :, :, :].squeeze().detach().cpu().numpy() * 255).astype('uint8')
        img = Image.fromarray(img)
        img.save(rf"..\BraTS2020\test_results\temp\{type}_{name}.jpg")

    @torch.no_grad()
    def p_sample_loop(self, shape, cond, note=None, return_history=False, ):

        img = torch.randn(shape, device=self.device)
        # img = note
        steps = torch.linspace(1.0, 0., self.num_sample_steps + 1, device=self.device)
        steps_cond = torch.linspace(0.001, 0., self.num_sample_steps, device=self.device)
        T = cond[:, -1:, :, :]
        RGB = cond[:, :-1, :, :]
        ...
        for i in range(self.num_sample_steps):
            ...
            # RGB_noised, _ = self.q_sample(RGB, steps_cond[i])
            # T_noised, _ = self.q_sample(T, steps_cond[i])
            img, x0 = self.p_sample(img, cond, times, times_next, return_history)
            # self.save_img(img, i, type="temp")
            # self.save_img(T, str(i) + "_cond", type="temp")
            # self.save_img(T_noised, str(i) + "_cond", type="temp")
        img.clamp_(-1., 1.)
        img = unnormalize_to_zero_to_one(img)
        return img

    @torch.no_grad()
    def sample(self, shape, cond, note=None):
        self.history = []
        self.p_sample_loop(shape, cond, return_history=True, note=note)
        return self.history
    ...
    def q_sample(self, x_start, times, noise=None, cond=None):
        log_snr = self.log_snr(times)
        log_snr_padded = right_pad_dims_to(x_start, log_snr)
        noise = default(noise, lambda: torch.randn_like(x_start))
        # _sam_mask = F.interpolate(cond, noise.shape[2:], mode='bilinear')
        to_torch = partial(torch.tensor, dtype=torch.float32)
        self.mask_coefficient = to_torch(np.sqrt(1. - log_snr_padded.sigmoid().clone().to("cpu")) * log_snr_padded.clone().to("cpu"))
        cond *= self.mask_coefficient.to(cond.device)
        # cond *= self.extract(self.mask_coefficient.to(cond.device), times, x_start.shape)
        noise += cond
        # log_snr = self.log_snr(times)
        ...
        self.save_img(x_noised,"x_noised","test")

        return x_noised, log_snr, noise

    def p_losses(self, x_start, times, noise=None,depth=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
