...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, cond, time, time_next, return_history=False):
        ...
        pred1, pred2, pred = self.model(x, batch_log_snr, cond)
        pred = pred1
    ...
    def p_sample(self, x, cond, time, time_next, return_history=False):
        ...
        model_mean, model_variance = self.p_mean_variance(x=x, cond=cond, time=time, time_next=time_next,
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, return_history=False):
        ...
        for i in range(self.num_sample_steps):
            ...
            img = self.p_sample(img, cond, times, times_next, return_history)
    ...
    def sample(self, shape, cond):
        self.history = []
        self.p_sample_loop(shape, cond, return_history=True)
        return self.history
